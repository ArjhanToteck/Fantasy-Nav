...
func draw_icon(element: OsmElement, extra_tags: Dictionary = {}) -> void:
	...
	if icon_texture != null:
		...
		if element is OsmWay:
			...
			draw_point = parent_map.world_to_game_position(element.latitude, element.longitude, min_latitude, min_longitude)
...
func get_points_from_way(way: OsmWay) -> Array:
	...
	for node in way.node_children:
		points.append(parent_map.world_to_game_position(node.latitude, node.longitude, min_latitude, min_longitude))
	return points
